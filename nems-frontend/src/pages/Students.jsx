...
function StudentDetail({ student, onClose }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }} onClick={onClose}>
      <div className="card" style={{ maxWidth: "560px", width: "100%", padding: "32px" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-4 mb-6">
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: "linear-gradient(135deg, var(--primary-500), var(--primary-400))", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <User size={28} color="#fff" />
          </div>
          <div>
            <div style={{ fontSize: "20px", fontWeight: 700, color: "var(--text-primary)" }}>{student.first_name} {student.last_name}</div>
...
export function StudentList() {
  ...
  useEffect(() => {
    api.getStudents().then(setStudents).catch((e) => toast.error(e.message)).finally(() => setLoading(false));
  }, []);
    ...
    <div>
      <div className="page-header">
        ...
        <div className="flex gap-2 items-center">
          ...
          <a href="/students/add" className="btn btn-primary"><UserPlus size={15} /> Add Student</a>
        </div>
      </div>
      <div className="page-body">
        <div className="flex gap-3 items-center mb-4">
          <div className="search-bar">
            ...
            <input className="search-input" placeholder="Search by name, email, ID..." value={query} onChange={(e) => setQuery(e.target.value)} />
          </div>
          <select className="form-select" style={{ width: "auto", minWidth: "140px" }} value={catFilter} onChange={(e) => setCatFilter(e.target.value)}>
            {["All", "General", "OBC", "SC", "ST", "EWS"].map((c) => <option key={c}>{c}</option>)}
          </select>
        </div>
        {loading ? <div style={{ padding: "40px", textAlign: "center", color: "var(--text-muted)" }}>Loading...</div> : (
          <div className="table-wrapper">
            <table className="data-table">
              <thead>
                <tr><th>ID</th><th>Name</th><th>Gender</th><th>DOB</th><th>Email</th><th>Phone</th><th>Category</th><th>Nationality</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={9}><div className="empty-state"><div className="empty-state-title">No students found</div></div></td></tr>
                ) : filtered.map((s) => (
                  <tr key={s.student_id}>
                    <td className="font-mono text-xs" style={{ color: "var(--text-secondary)" }}>#{s.student_id}</td>
                    <td>
                      <div className="flex items-center gap-2">
                        <div style={{ width: 30, height: 30, borderRadius: "50%", background: "linear-gradient(135deg, var(--primary-400), var(--primary-300))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                    ...
                    <td><button className="btn btn-ghost btn-sm" onClick={() => setSelected(s)}><Eye size={13} /> View</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export function AddStudent() {
  const [form, setForm] = useState({
    ...
    email: "", phone: "", address: "", nationality: "Indian", category: "General",
  });
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-title">Add Student</div><div className="page-subtitle">Register a new student in the system</div></div>
      </div>
      <div className="page-body">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-grid mb-4">
                ...
                <div className="form-group">
                  ...
                  <select className="form-select" {...field("gender")}><option value="M">Male</option><option value="F">Female</option><option value="O">Other</option></select>
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <select className="form-select" {...field("category")}>{["General", "OBC", "SC", "ST", "EWS"].map((c) => <option key={c}>{c}</option>)}</select>
                </div>
              </div>
              ...
              <div className="flex gap-3 justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button type="button" className="btn btn-ghost" onClick={() => setForm({ first_name: "", last_name: "", date_of_birth: "", gender: "M", email: "", phone: "", address: "", nationality: "Indian", category: "General" })}>Reset</button>
                  <button type="submit" className="btn btn-primary btn-lg" disabled={loading}>{loading ? "Saving..." : "Add Student"}</button>
                </div>
              </div>
            </form>
          </div>
